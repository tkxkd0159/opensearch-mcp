...
class OpenSearchPropertiesTest {
    ...
    runner
        ...
        .run(
            ctx -> {
              ...
              assertThat(props.getClusters().get("local").getUrl())
                  .isEqualTo("https://localhost:9200");
              assertThat(props.getClusters().get("prod").getUrl())
                  .isEqualTo("https://prod.example.com:9200");
            });
  }
  ...
  void writeEnabled_canBeDisabled() {
    runner
        .withPropertyValues("opensearch.write-enabled=false")
        .run(
            ctx -> {
              assertThat(ctx).hasNotFailed();
              OpenSearchProperties props = ctx.getBean(OpenSearchProperties.class);
