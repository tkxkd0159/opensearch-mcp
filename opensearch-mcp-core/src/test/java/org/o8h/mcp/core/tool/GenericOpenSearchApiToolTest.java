...
class GenericOpenSearchApiToolTest {
  ...
  void callApi_get_withQueryParamsAndHeaders_dispatches() {
    mockServer
        .expect(requestTo("http://localhost:9200/_cat/shards?v=true"))
  ...
  void callApi_post_withBody_whenWritesEnabled_succeeds() {
    mockServer
        .expect(requestTo("http://localhost:9200/_search"))
  ...
  void callApi_opensearchErrorResponse_returnsBodyAsString() {
    String errorBody =
        "{\"error\":{\"type\":\"index_not_found_exception\",\"reason\":\"no such index [logs]\"},\"status\":404}";
    mockServer
        .expect(requestTo("http://localhost:9200/logs/_search"))
        .andExpect(method(org.springframework.http.HttpMethod.GET))
        .andRespond(
